<template>
    ...
    mode: 'fixedSize',
        ...
        <div class="space-y-2 border p-2 rounded-lg">
            <div v-if="imageReviewUrl" class="lg:h-[500px]  rounded-lg overflow-hidden bg-gray-100">
                <img class="h-full w-full aspect-square object-cover" :src="imageReviewUrl" alt="review" />
            </div>
            <div class="flex flex-row items-center space-x-4">
                <div>
                    <button type="button" class="btn btn-ghost btn-outline normal-case" @click="open()" v-if="!image">

                        <span> {{ imageReviewUrl ? "Change Image" : "Upload Image" }} </span>
...
const imageReviewUrl = computed(() => {
    return imageReview.value ? URL.createObjectURL(imageReview.value) : props.existingimage ?? null;
});
