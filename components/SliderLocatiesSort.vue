        ...
        <ClientOnly>
          ...
          >
            <swiper-slide
              ...
              :key="itemSlider.id"
              :style="{
                borderColor: itemSlider?.product?.level_type?.color,
                borderWidth: '2px',
                borderStyle: 'solid',
              }"
            >
              <img
                :src="
                  itemSlider?.product?.images[0]?.image ||
                  itemSlider?.product?.images[1]?.image ||
                  itemSlider?.product?.images[2]?.image ||
                  itemSlider?.product?.images[3]?.image
                    ? itemSlider?.product?.images[0].image
              ...
              />
              <NuxtLink
                :to="`/onze-locaties/${itemSlider?.product?.slug}`"
              ...
              >
                <div class="flex flex-col items-center gap-2">
                  <h2 class="text-md lg:text-2xl font-semibold tracking-wider">
                    {{ itemSlider?.product?.location?.name }}
                  </h2>
                ...
                <div
                  class="flex flex-col justify-center items-center mt-4 gap-2"
                >
                  <h4 class="text-[16px] lg:text-lg font-semibold">
                    {{ itemSlider?.product?.area_size }} m<sup>2</sup>
                  </h4>
                  <p class="text-[15px] lg:text-sm">
                    € {{ itemSlider?.product?.price }} p/{{
                      formattedRentType(itemSlider?.product?.rent_type)
                    }}
                  </p>
