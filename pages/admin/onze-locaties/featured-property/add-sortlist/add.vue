      ...
      <div class="flex justify-between items-center">
        <div>
          <div class="text-lg md:text-2xl font-bold">
            <CompAdminBackButton
              link="onze-locaties/featured-property/add"
              linkTitle="Sort list Product Slider"
            />
          </div>
        </div>
      </div>
          ...
          <table class="table table-xs md:table-md w-full rounded-t-xl">
            <thead class="h-12">
              <tr>
                ...
                <th class="font-medium">Edit / Add list</th>
              </tr>
            </thead>
            <tbody>
              <tr
                  ...
                  <dialog :id="'my_modal_' + index" class="modal">
                    <div class="modal-box">
                      <h3 class="font-bold text-lg">
                        Edit Position Sort of {{ item?.name }}?
                      </h3>
                      <input
                        :id="item?.name"
                        type="number"
...
const { data: sliderData } = useFetch(`/admins/products`, {
  method: "get",
  ...requestOptions,
});
