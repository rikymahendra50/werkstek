<template>
  <CompAdminBackButton link="onze-vacatures" linkTitle="Edit Vacatures" />
  <div class="overflow-y-auto grid md:grid-cols-2">
    <Form
      @submit="onSubmit"
      v-slot="{ error }"
      :validation-schema="editVacaturesSchema"
...
const { editVacaturesSchema } = useSchema();
...
const typesArray = job?.value?.data?.types.map((item) => item?.type_id);
const tagsArray = job?.value?.data?.tags.map((item) => item?.name);
...
const formData = ref({
  ...
  category: job?.value.data?.category,
  tags: tagsArray.map((name) => ({
    name: name,
  })),
});
...
async function onSubmit(values, ctx) {
  try {
    ...
    const response = await axiosRequest.post(
      `/admins/jobs/${slug.value}?_method=PUT`,
      formDataT
    );
